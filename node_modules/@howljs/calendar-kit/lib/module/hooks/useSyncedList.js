...
import { runOnJS, runOnUI, scrollTo, useAnimatedScrollHandler, useSharedValue } from 'react-native-reanimated';
...
const useSyncedList = ({
  ...
  const _onMomentumEnd = () => {
    if (isTriggerMomentum.current && startDateUnix.current !== visibleDateUnix.current) {
      ...
      onDateChanged === null || onDateChanged === void 0 || onDateChanged(dateTimeToISOString(parseDateTime(visibleDateUnix.current)));
  ...
  const onVisibleColumnChanged = useCallback(props => {
    const {
      index: pageIndex,
      column,
      columns,
