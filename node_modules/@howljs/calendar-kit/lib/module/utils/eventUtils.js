import { DEFAULT_MIN_START_DIFFERENCE, MILLISECONDS_IN_DAY, MILLISECONDS_IN_MINUTE, MINUTES_IN_DAY } from '../constants';
...
export const filterEvents = (events, minUnix, maxUnix, useAllDayEvent) => {
  const allDays = [];
  const regular = [];
...
export const buildInstanceId = (id, date) => {
  return `${id}_${date.toUTC().toFormat("yyyyMMdd'T'HHmmss'Z'")}`;
};
export const divideEvents = (event, timeZone, minRegularEventMinutes) => {
  const events = [];
  ...
  for (let i = 0; i < days; i++) {
    ...
    const dateObj = parseDateTime(startUnix).plus({
      days: i
    });
...
const calculateVisibleDuration = (startUnix, endUnix, timeZone, hideWeekDays) => {
  ...
  while (currentUnix <= endUnix) {
    const dateTime = parseDateTime(currentUnix, {
      zone: timeZone
    });
...
export const divideAllDayEvents = (event, timeZone, firstDay, hideWeekDays) => {
  const events = [];
  ...
  const eventEndUnix = isSameDay ? eventStart.endOf('day').toMillis() : parseDateTime(eventEnd.toMillis() - 1).endOf('day').toMillis();
  if (diffWeeks <= 1) {
...
export function processEventOccurrences(event, minUnix, maxUnix, timeZone, divideFunction) {
  if (event.recurrence) {
    const rrule = new RRuleGenerator(event.recurrence, parseDateTime(event.start.dateTime || event.start.date, {
      zone: event.start.timeZone
    }), event.excludeDates);
    const occurrences = rrule.generateOccurrences(parseDateTime(minUnix, {
      zone: timeZone
    }), parseDateTime(maxUnix, {
      zone: timeZone
    }));
...
export function processAllDayEventMap(allDayEventMap, timeZone, hideWeekDays) {
  ...
  allDayEventMap.forEach((eventsForWeek, weekStart) => {
    ...
    const {
      ...
      startDate: weekStart,
      endDate: parseDateTime(weekStart).plus({
        days: 6
      }).endOf('day').toMillis(),
      timeZone,
      visibleDays
    ...
    packedEvents.forEach(event => {
      ...
      for (let dayUnix = eventStart; dayUnix <= eventEnd; dayUnix = parseDateTime(dayUnix).plus({
        days: 1
      }).toMillis()) {
...
export function getVisibleDays(weekStart, timeZone, hideWeekDays) {
  const visibleDays = [];
  let currentDayUnix = weekStart;
  for (let i = 0; i < 7; i++) {
    const dateTime = parseDateTime(currentDayUnix, {
      zone: timeZone
    });
    const weekday = dateTime.weekday;
    if (!hideWeekDays.includes(weekday)) {
...
export const populateEvents = (events, {
  ...
  const handleEvents = (eventsToHandle, resourceIndex) => overlap ? handleOverlap(eventsToHandle, minStartDifference * MILLISECONDS_IN_MINUTE, resourceIndex) : handleNoOverlap(eventsToHandle, resourceIndex);
...
export const populateAllDayEvents = (events, options) => {
  const sortedEvents = sortEvents(events);
  const rows = [];
