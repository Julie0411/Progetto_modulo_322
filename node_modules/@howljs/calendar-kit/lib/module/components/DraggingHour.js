...
const DraggingHourInner = ({
  ...
  useAnimatedReaction(() => roundedDragStartMinutes.value, (value, prev) => {
    if (prev !== value) {
      runOnJS(setStartMinutes)(value);
    }
  });
  useAnimatedReaction(() => roundedDragStartMinutes.value + roundedDragDuration.value, (value, prev) => {
    if (prev !== value) {
  ...
  const startHourStr = useMemo(() => {
    return startMinutes !== -1 ? toHourStr(startMinutes, hourFormat, locale.meridiem) : '';
  }, [startMinutes, hourFormat, locale.meridiem]);
  const endHourStr = useMemo(() => {
    return endMinutes !== -1 ? toHourStr(endMinutes, hourFormat, locale.meridiem) : '';
  ...
  }, startHourStr)), /*#__PURE__*/React.createElement(Animated.View, {
    pointerEvents: "box-none",
    style: [styles.absolute, {
      width: hourWidth - HOUR_SHORT_LINE_WIDTH - 8 - lineWidth
    }, endAnimStyle]
...
const DraggingHour = ({
  ...
  return /*#__PURE__*/React.createElement(DraggingHourInner, {
    renderHour: renderHour
  });
};
