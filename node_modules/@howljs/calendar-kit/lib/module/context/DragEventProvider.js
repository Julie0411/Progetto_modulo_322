...
import { runOnJS, runOnUI, scrollTo, useAnimatedReaction, useSharedValue } from 'react-native-reanimated';
...
const DragEventContext = /*#__PURE__*/React.createContext(undefined);
const DragEventActionsContext = /*#__PURE__*/React.createContext(undefined);
...
const DragEventProvider = ({
  ...
  const dragStartUnix = useSharedValue(-1);
  const dragStartMinutes = useSharedValue(-1);
  const dragDuration = useSharedValue(-1);
  const roundedDragStartUnix = useSharedValue(-1);
  const roundedDragStartMinutes = useSharedValue(-1);
  const roundedDragDuration = useSharedValue(-1);
  const dragX = useSharedValue(-1);
  const extraMinutes = useSharedValue(0);
  const dragSelectedType = useSharedValue(undefined);
  ...
  const dragSelectedStartUnix = useSharedValue(-1);
  const dragSelectedStartMinutes = useSharedValue(-1);
  const dragSelectedDuration = useSharedValue(-1);
  const isDraggingSelectedEvent = useSharedValue(false);
  const isDraggingCreateAnim = useSharedValue(false);
  const autoHScrollTimer = useRef();
  const autoVScrollTimer = useRef();
  ...
  useAnimatedReaction(() => isDraggingAnim.value, (dragging, prevDragging) => {
  ...
  useAnimatedReaction(() => roundedDragStartMinutes.value, (clampedStart, prevStart) => {
    if (clampedStart !== prevStart && clampedStart !== -1) {
      runOnJS(_triggerHaptic)();
    }
  });
  useAnimatedReaction(() => roundedDragStartUnix.value, (unix, prevUnix) => {
    if (unix !== prevUnix && unix !== -1) {
      runOnJS(_triggerHaptic)();
    }
  });
  useAnimatedReaction(() => roundedDragDuration.value, (duration, prevDuration) => {
  ...
  useEffect(() => {
    ...
    if (selectedEvent !== null && selectedEvent !== void 0 && (_selectedEvent$start = selectedEvent.start) !== null && _selectedEvent$start !== void 0 && _selectedEvent$start.dateTime && selectedEvent !== null && selectedEvent !== void 0 && (_selectedEvent$end = selectedEvent.end) !== null && _selectedEvent$end !== void 0 && _selectedEvent$end.dateTime) {
      ...
      dragSelectedStartUnix.value = parseDateTime(eventStart.toISODate()).toMillis();
  ...
  useAnimatedReaction(() => dragPosition.value.x, (curX, prevX) => {
  ...
  useAnimatedReaction(() => dragPosition.value.y, (dragY, prevY) => {
  ...
  const triggerDragSelectedEvent = useCallback(initialDrag => {
    if (!selectedEvent) {
      return;
    }
    ...
    runOnUI(() => {
      if (initialDrag.startIndex === 0) {
        ...
        const diffDays = Math.floor((dragSelectedStartUnix.value - visibleDateUnixAnim.value) / MILLISECONDS_IN_DAY);
        ...
        const startByIndex = dragSelectedStartUnix.value + initialDrag.startIndex * MILLISECONDS_IN_DAY;
        const diffDays = Math.floor((startByIndex - visibleDateUnixAnim.value) / MILLISECONDS_IN_DAY);
        const startUnix = visibleDateUnixAnim.value + diffDays * MILLISECONDS_IN_DAY;
        dragStartUnix.value = startUnix;
        roundedDragStartUnix.value = startUnix;
        const originalStart = dragSelectedStartUnix.value + dragSelectedStartMinutes.value * MILLISECONDS_IN_MINUTE;
  ...
  const triggerDragEvent = useCallback((initialDrag, event) => {
    ...
    runOnUI(() => {
      if (initialDrag.startIndex === 0) {
        dragStartMinutes.value = startMinutes;
        roundedDragStartMinutes.value = startMinutes;
        const diffDays = Math.floor((eventStartUnix - visibleDateUnixAnim.value) / MILLISECONDS_IN_DAY);
        const startUnix = visibleDateUnixAnim.value + diffDays * MILLISECONDS_IN_DAY;
        dragStartUnix.value = startUnix;
        roundedDragStartUnix.value = startUnix;
      } else {
  ...
  const triggerDragCreateEvent = useCallback((date, event) => {
  ...
  return /*#__PURE__*/React.createElement(DragEventContext.Provider, {
    value: contextValues
  }, /*#__PURE__*/React.createElement(DragEventActionsContext.Provider, {
